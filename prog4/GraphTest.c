...
int main(int argc, char* argv[]){
   ...
   //tests the addEdge and addArc function by nature of my code
   addEdge(G, 9, 31);
   addEdge(G, 17, 13);
   addEdge(G, 14, 33);
   ...
   for(s=1; s<=n; s++){
      //BFS tests getPath and getParent by nature of code
      BFS(G, s);
      max = getDist(G, 1);
      for(i=2; i<=n; i++){
       	 d = getDist(G, i);
   ...
   //this tests the int return functions.
   printf("Source is: %i\n", getSource(G));
   printf("Number of edges are: %i\n",getSize(G));
   printf("Number of vertices are: %i\n",getOrder(G));
   ...
   printf("\n");
   makeNull(G);
   if(G == NULL) printf("Graph is NULL");
