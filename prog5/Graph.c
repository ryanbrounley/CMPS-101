...
/**Constructors - Destructors */
	...
	if(n < 0){
		fprintf(stderr, "n is not a valid non-negative integer");
		exit(1);}	
	...
	for(int i = 1; i <= n; i++){
		...
		newGraph->discover[i] = INF;
		newGraph->finish[i] = INF;
	}
	return newGraph;	
    }
...
/**Access functions**/
	...
	if(G == NULL){
		fprintf(stderr, "G is NULL getOrder()");
		exit(1);}
	return G->verts;
    }
    ...
    int getSize(Graph G){
	if(G == NULL){
		fprintf(stderr, "G is NULL getSize()");
		exit(1);}
	return G->edges;
    }
	...
	if(G == NULL){
		fprintf(stderr, "G is NULL getParent()");
		exit(1);}	
	...
	if(G == NULL){
		fprintf(stderr, "G is NULL getDist()");
		exit(1);}	
	else if(getSource(G) == NIL) return INF;
	else if(1 > u || u > getOrder(G)){ 
    ...
    //returns the discover time of the vertex u in graph G
    ...
    //returns the finish time of the vertex u in graph G
    int getFinish(Graph G, int u){
	if(G == NULL){
		fprintf(stderr, "G is NULL getFinish()");
		exit(1);}	
	else if(1 > u && u > getOrder(G)){ 
		fprintf(stderr, "u is not a valid part of the graph G getParent() "); 
		exit(1);}
...
/**Manipulation procedures**/
	...
	if(G == NULL){
		fprintf(stderr, "G is NULL makeNULL()");
		exit(1);}	
	...
	if(G == NULL){
		fprintf(stderr, "G is NULL addEdge()");
		exit(1);}	
	else if(0 > u || u > getOrder(G)){
		fprintf(stderr, "u is out of Graph boundries addEdge() ");
		exit(1);}
	else if(0 > v || v > getOrder(G)){
	...
	if(G == NULL){
		fprintf(stderr, "G is NULL addArc()");
		exit(1);}	
	else if(1 > u || u > getOrder(G)){
    ...
    //Runs DFS through the stack S using the visit helper function below. Manipulates the stack to the order of finish
    void DFS(Graph G, List S){
	if(G == NULL){
		fprintf(stderr, "G is NULL DFS()");
		exit(1);}	
	...
	for(int i = 1; i <= getOrder(G); i++){
		...
		if(G->color[getElement(S)] == WHITE){
		 	visit(G, S, getElement(S));	
		}
    	}
	for(int d = 1;d <= getOrder(G); d++) deleteBack(S);
    }

    //DFS helper function keeps track of discovers and finishes
    void visit(Graph G, List S, int x){
	if(G == NULL){
		fprintf(stderr, "G is NULL visit()");
		exit(1);}
	G->color[x] = GRAY;
	G->discover[x] = ++G->time;
	List adjList = G->adj[x];
	int tmp;
	for(moveTo(adjList, 0); getIndex(adjList) > -1; moveNext(adjList)){
		tmp = getElement(adjList);
		if(G->color[tmp] == WHITE){
			G->parent[tmp] = x;
			visit(G, S, tmp);
		}
	}
    ...
    /** Other operations **/
    //flips the directions of the edges 
    Graph transpose(Graph G){
	if(G == NULL){
		fprintf(stderr, "G is NULL transpose()");
		exit(1);}
	...
	for(int i = 1; i <= getOrder(G); i++){
		tempList = G->adj[i];
		for(moveTo(tempList, 0); getIndex(tempList) > -1; moveNext(tempList)){
			tmp = getElement(tempList);
			append(trans->adj[tmp], i);	
		}
	}
	return trans;
    }

    //takes the graph G and copys it a new graph exactly and returns
    Graph copyGraph(Graph G){
	if(G == NULL){
		fprintf(stderr, "G is NULL copyGraph()");
		exit(1);}
	...
	if(G == NULL){
		fprintf(stderr, "G is NULL printGraph()");
		exit(1);}
	for(int i = 1; i <= G->verts; i++){
		fprintf(out, "%d: ", i);
